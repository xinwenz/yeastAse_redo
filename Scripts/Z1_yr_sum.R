...
x_t <- colSums(calh_1[,grep(".*_yGc",names(calh_1),value=T)],na.rm=T)
y_t <- colSums(calh_1[,grep(".*_rGc",names(calh_1),value=T)],na.rm=T)
xy <- data.frame(x_t,y_t)

ggplot(xy,aes(x=x,y=y)) + 
    geom_point(size=3) +
    labs(title="Mapping of 20 replicates using two references \n without suspected positions, merged for each chromsome!" , x = "total read counts mapped to Yps128 allele (million counts)" , y = "total read counts mapped to Rm11-1a allele (million counts)" ) + 
