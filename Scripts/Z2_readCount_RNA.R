setwd("~/mnt/nnp/2_0_2_express_hisat2/yps5rmB_geneCount/")
fileNum <- sapply(strsplit(x=list.files(path="./", pattern="*.geneCount"),split = "_"),"[",1)
fileLab <- rep(c("A","H"),times=10)
smpID <- paste0(fileNum,fileLab)
file <- paste0(fileNum,"_", fileLab,".geneCount")
...
xy <- data.frame(x,y)
p <- ggplot(xy,aes(x=x,y=y)) + geom_point() + labs(title="Expression read counts" , x = "total Expression read counts mapped to Yps128 allele" , y = "total read counts mapped to Rm11-1a allele" ) + scale_y_continuous(labels = scales::scientific) + scale_x_continuous(labels = scales::scientific) + geom_abline(slope = 1,intercept = 0) + theme(text=element_text(size=15))+coord_equal()
...
save(exph, file="./Z2_out/exph.RData")
ggsave(p, file="./Z2_out/Expressio_reads_sum.pdf")
