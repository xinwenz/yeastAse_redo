...
for(i in 1:450) {
    mydataName <- paste0("~/mnt/nnp/3_0_1_infer_ase/exph_redo/exph_ci_",formatC(i,width=3,flag="0"),".RData")
    load(mydataName)
}

df_exph <- fpo_format(nm="exph_ci_",each_nr = 25,nr = 18, nGenes= 4636,nbit=3)
...
p <- ggplot(df_exph, aes(x=Used_replicates, y=fp_rate, shape=type)) + 
...
ggsave(p,file= "~/cloud/yeastAse_redo/Z4_out/fig_exph.pdf", device = "pdf", width = 20, height = 15, units = "cm",dpi=300)
